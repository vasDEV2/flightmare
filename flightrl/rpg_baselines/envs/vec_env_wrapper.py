...
class FlightEnvVec(VecEnv):
    ...
    def __init__(self, impl):
        ...
        self.obs_cop = np.zeros((self.num_envs, 18))
        ...
        self._goal = np.ones((self.num_envs, 3))
    
        self._goal[:, :3] = np.array([0, 0, 0.5])
    ...
    def step(self, action):
        ...
        if not np.isfinite(self._observation).all(): raise ValueError(f"NaN or Inf detected in input: {self._observation}")

        # euler = self._observation[:, 3:6]
        # r = R.from_euler("zyx", euler)
        # rotation = r.as_matrix()
        # rotation = rotation.reshape(50, 9, order='C')
        # print("PAIJAAAN")


        # # print(quats)
        # print("11111", self._observation[1])
        ...
        for i in range(self.num_envs):
            ...
            if self._done[i]:
                ...
                # print(f"EP_LEN{eplen} | EP_REWARD {eprew}")
                epinfo = {"r": eprew, "l": eplen}
                info[i]['episode'] = epinfo
                self.rewards[i].clear()

        # print(f"OBSERVATION RAW = {self._observation}")
        
        # print("222222", self._observation[1])

        # self._observation[:, :3] = self._goal - self._observation[:, :3]
        # print("YOUOOOOO")
        # print(self._observation)
        # self.obs_cop[:, 0:3] = self._observation[:, 0:3]
        # self.obs_cop[:, 3:12] = rotation
        # self.obs_cop[:, 12:18] = self._observation[:, 6:12]

        # print(f"OBSERVATION = {self._observation.shape}")
    ...
    def reset(self):
        # print("RESETTING")
        self._reward = np.zeros(self.num_envs, dtype=np.float32)
        # print("YOOOOO", self._observation[1])
        self.wrapper.reset(self._observation)
        # print("YOOOOOOO@", self._observation[1])
